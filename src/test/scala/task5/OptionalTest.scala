...
class OptionalTest:
  ...
  @Test def filterShouldReturnTheValueWhenThePredicateIsSatisfied(): Unit = {
    val nonEmpty = Optional.Maybe(10)
    val result = Optional.filter(nonEmpty, _ > 0)
    assertEquals(10, Optional.orElse(result, 1))
  }

  @Test def filterShouldReturnEmptyWhenThePredicateIsNotSatisfied(): Unit = {
    val nonEmpty = Optional.Maybe(0)
    val result = Optional.filter(nonEmpty, _ > 0)
    assertTrue(Optional.isEmpty(result))
  }